...
Given('Estoy logueado', async ({ loginPage, ctx, auth, $workerInfo, $testInfo}) => {
    ctx.variableDelCtx ="Soy un valor en el contexto";
...
When('Agrego un producto al carrito', async ({homePage, productPage, cartPage, page, $testInfo, ctx, world}) => {
    log.debug(`Valor en el contexto: ${ctx.variableDelCtx}`);
